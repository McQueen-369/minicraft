...
/** Robot worlds grow no apples, so their rations come in tins instead. */
function ration(kind: WorldKind): number {
  return kind === 'robot' ? ItemId.CannedFood : ItemId.Apple
}

/** Deterministic starter loot for a naturally generated chest. */
export function chestLoot(seed: number, x: number, z: number, kind: WorldKind = 'terrain'): ChestContents {
  ...
  put(ration(kind), 2 + Math.floor(r(15) * 4))
...
export function mysteryBoxLoot(blockId: number, kind: WorldKind = 'terrain'): ChestContents {
  ...
  if (blockId === BlockId.MysteryBoxEpic) {
    ...
    contents.push({ itemId: ration(kind), count: 5 + Math.floor(rand() * 5) })
    ...
    contents.push({ itemId: ration(kind), count: 3 + Math.floor(rand() * 4) })
